...
import csv
from stories import all_movies, liked_movies, not_liked_movies, did_not_watch
...
def get_movie():
    movie_data = {
        "title": all_movies[0][19], # 0th row 19 column
...
def liked_movies():
    ...
    #cop = remove that movie from all movies list
    all_movies.cop(0)
...
def recommended_movies():
    ...
    # lets us work on multiple functions easily
    import itertools

    all_recommended.sort()
    all_recommended = list(all_recommended for all_recommended,_ in itertools.groupby(all_recommended))#grouping all recommended
    movie_data = []
